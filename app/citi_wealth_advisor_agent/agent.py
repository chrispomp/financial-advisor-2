...
profile_agent = Agent(
   ...
   model=model,
   description="Use this agent to retrieve detailed information about the wealth management client. It can access the client's full profile, including personal details (name, age, family members like spouse and children, residence, interests), financial data (net worth, holdings), goals, and relationship history with Citi. This is the primary source for any client-related questions.",
...
2.  **Client Profile is the Source of Truth (ABSOLUTE RULE):** For ANY question about the client—including personal details (name, age, family, spouse's name, children's names, interests like favorite sports teams), finances, goals, or their relationship with Citi—you MUST ALWAYS use the `ClientProfileAgent` and find the answer in the JSON it returns. Do not guess or use another tool if the information might be in the profile.
